...
- 2026-09-12: **Fix WhatsApp / Shortcut Base64 Sanitization for Gemini OCR** — WhatsApp / iOS Shortcut shares previously failed Google Gemini API with HTTP 400 due to standard MIME line breaks and whitespace in the Base64 image payload, silently falling back to OpenAI GPT-4o. Added Base64 sanitization (stripping `\r\n` and spaces, URL-decoding) in `api/scan-ticket.js` and updated model chain (`gemini-2.5-flash`, `gemini-2.5-flash-lite`, `gemini-2.5-pro`) so Gemini processes all WhatsApp shares directly.
- 2026-09-12: **Unified 12PK Dozen Rule across Camera & WhatsApp Share** — All non-birthday items (slices, frosted, tres leches, family size, square/cornbread) with "12PK" or `unit="dozen"` are converted to units (×12, e.g. 0.5 = 6 units, 1 = 12 units). Birthday cakes remain strictly 1:1 whole cakes. Applied consistently across Gemini prompt, admin dashboard, and driver order forms.
