...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        window.staffPermissions = profile.role === 'admin' ? {
        ...
        if (!window.staffPermissions.can_view_orders) document.getElementById('tab-orders')?.remove();
        if (!window.staffPermissions.can_approve_partners) document.getElementById('tab-partners')?.remove();
        if (!window.staffPermissions.can_manage_users) document.getElementById('tab-users')?.remove();
...
window.approvePartner = async function(partnerId, btnEl) {
    if (!window.staffPermissions?.can_approve_partners) {
        alert('Access Denied: You do not have permission to approve partners.');
        return;
    }
    btnEl.disabled = true;
...
window.handleAdvanceClick = async function(btnElem, orderId, currentStatus, fromModal = false) {
    ...
    if (!window.staffPermissions?.can_advance_orders) {
        alert('Access Denied: You do not have permission to advance orders.');
        return;
    }
...
window.handleCancelClick = async function(btnElem, orderId, fromModal = false) {
    ...
    if (!window.staffPermissions?.can_cancel_orders) {
        alert('Access Denied: You do not have permission to cancel orders.');
        return;
    }
